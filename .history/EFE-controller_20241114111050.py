...
query = f"SELECT * FROM \"nome_misurazione\" WHERE \"machine_id\"='{machine_id}' LIMIT 1"
params = {
    "db": "tc_db",  # Specifica il database
    "q": f'SELECT "value" FROM "tc_db"."autogen"."rate" WHERE time > now() - 1h AND "flowid" != \'1\' AND "flowid" != \'2\' AND "flowid" != \'3\' AND "machineid" = \'{machine_id}\''
}
