...
def bpftool_map_update(map_reference, key, value, map_reference_type="pinned", value_type="pinned"):
    ...
    if map_reference_type == "pinned":
        ...
        if isinstance(value, int):
            ...
            value_string = " ".join(hex(v) for v in value)
        else:
            raise TypeError(f"Expected 'value' to be an int or list, got {type(value)}")

        if value_type == "pinned":
            cmd = f"bpftool map update pinned {map_reference} key hex {key_string} value pinned {value_string}"
        elif value_type == "hex":
