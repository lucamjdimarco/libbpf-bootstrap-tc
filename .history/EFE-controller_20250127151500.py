...
from settings import BPF_FS_PATH, INFLUXDB_URL_IPV6, INFLUXDB_URL_IPV4, REDIS_HOST_IPV4, REDIS_HOST_IPV6
...
protocol = sys.argv[2]
type_of_classifier = int(sys.argv[3])
...
def listen_to_redis():
    ...
    client = redis.StrictRedis(host=REDIS_HOST_IPV6, port=6379, decode_responses=True)
...
def parse_map_dump_to_json(dump_data, classifier):
    ...
    try:
        ...
        if classifier == 1:
            ...
            return handle_ipv6_quintuple(flow_id, value)
        elif classifier == 3:
            return handle_ipv4_only(flow_id, value)
        elif classifier == 4:
            return handle_ipv6_only(flow_id, value)
        elif classifier == 5:
            return handle_ipv4_dest_only(flow_id, value)
        elif classifier == 6:
            return handle_ipv6_dest_only(flow_id, value)
        else:
...
def handle_ipv6_quintuple(flow_id, value):
...
def handle_ipv4_only(flow_id, value):
    src_ip = socket.inet_ntoa(struct.pack('<I', value.get("src_ip", 0)))
    dst_ip = socket.inet_ntoa(struct.pack('<I', value.get("dst_ip", 0)))
...
def handle_ipv6_only(flow_id, value):
    src_ip = socket.inet_ntop(socket.AF_INET6, struct.pack('<16s', value.get("src_ip", [0] * 16)))
    dst_ip = socket.inet_ntop(socket.AF_INET6, struct.pack('<16s', value.get("dst_ip", [0] * 16)))
...
def handle_ipv4_dest_only(flow_id, value):
    dst_ip = socket.inet_ntoa(struct.pack('<I', value.get("dst_ip", 0)))
...
def handle_ipv6_dest_only(flow_id, value):
    dst_ip = socket.inet_ntop(socket.AF_INET6, struct.pack('<16s', value.get("dst_ip", [0] * 16)))
