...
def reader(pipe, queue, source_name):
    ...
    try:
        for line in iter(pipe.readline, b''):  # Legge riga per riga
            if line:
                decoded_line = line.decode("utf-8", errors="replace").strip()  # Decodifica e rimuove newline
                print(f"Debug {source_name}: {decoded_line}")  # Log di debug
