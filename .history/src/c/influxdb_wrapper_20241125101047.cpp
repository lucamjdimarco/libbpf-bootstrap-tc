...
int InfluxDBWrapper::writeData(uint64_t ts, const TagInfluxDB &tags, uint64_t counter) {
    influxdb::Point point("rate");
	double ccnt = 1.0D * counter;
	
	point.addTag("machine_id", tags.machine_id);
	point.addTag("interface", tags.interface);
	point.addTag("flowid", std::to_string(tags.flowid));
