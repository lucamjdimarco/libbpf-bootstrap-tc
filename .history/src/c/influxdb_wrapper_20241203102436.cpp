...
int InfluxDBWrapper::writeData(uint64_t ts, const std::string& machine_id, 
	const std::string& interface, const std::string& fname, uint64_t flowid, uint64_t counter) {
    
	influxdb::Point point("rate");
	double ccnt = 1.0D * counter;
	
	point.addTag("machine_id", machine_id);
	point.addTag("interface", interface);
	point.addTag("friendlyname", fname);
	point.addTag("flowid", std::to_string(flowid));
