...
extern "C" {
	...
	int write_data_influxdb(MHandler_t *h,
			    uint64_t ts, const char *machine_id, const char *interface, uint64_t flowid, uint64_t counter);
	
	int write_data_influxdb_batch(MHandler_t *h, uint64_t *ts, const char *machine_id[BATCH_SIZE], const char *interface[BATCH_SIZE], uint64_t *flowid, uint64_t *counter, size_t count);
