...
int wait_for_flow_id() {
	u32 key = 0;
    int attempts = 0;
    int max_attempts = 10; // Numero massimo di tentativi
    int sleep_duration = 1; // Durata attesa in secondi
    
    while (attempts < max_attempts) {
        u64 *flow_id = bpf_map_lookup_elem(&flow_map, &key);
        
        if (flow_id && *flow_id > 0) {
            // Flow ID trovato e valido, procedi
            printf("Flow ID trovato: %llu\n", *flow_id);
            return 0;
        }

        // Se non trovato o non valido, attende e riprova
        printf("Flow ID non trovato, tentativo %d. Attendo...\n", attempts + 1);
        sleep(sleep_duration);  // Attendi prima di riprovare
        attempts++;
    }

    // Se il flow_id non è stato trovato dopo i tentativi massimi
