...
static __always_inline size_t safe_strncpy(char *dest, const char *src, size_t dest_size) {
    size_t i;
...
static __always_inline size_t safe_strncat(char *dest, const char *src, size_t dest_size) {
    ...
    if (dest_len + i < dest_size)
        dest[dest_len + i] = '\0'; // Null-termina

    return dest_len + i; // Restituisci la nuova lunghezza
}
