...
static __always_inline void bpf_safe_memcpy(char *dest, const char *src, size_t len) {
    for (size_t i = 0; i < len; i++) {
        dest[i] = src[i];
    }
}

static __always_inline void bpf_safe_strncpy(char *dest, const char *src, __u32 max_len) {
    for (__u32 i = 0; i < max_len - 1; i++) {
        if ((dest[i] = src[i]) == '\0')
            return;
    }
    dest[max_len - 1] = '\0';
