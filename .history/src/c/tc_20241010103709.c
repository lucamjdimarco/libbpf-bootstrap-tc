...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	if (events_count < BATCH_SIZE - 1){
		...
		for (int i = 0; i < events_count; i++){
			...
			/* ----- */
			int ret = write_data_influxdb(influx_handler, ((events_buffer[i].ts* 1000000000ULL) + start_of_the_kernel_abs), events_buffer[i].flowid, events_buffer[i].counter);
			...
			printf("Time of the packet: %llu\n", (events_buffer[i].ts* 1000000000ULL));
			printf("Abs time of the packet: %llu\n", (events_buffer[i].ts* 1000000000ULL) + start_of_the_kernel_abs));
			/* ----- */
			if (ret != 0) {
