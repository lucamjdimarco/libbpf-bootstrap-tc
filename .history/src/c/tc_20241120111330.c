...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	if (events_count < BATCH_SIZE - 1) {
		...
		char *str_identifiers[BATCH_SIZE];
		...
		for (int i = 0; i < events_count; i++) {
			...
			str_identifiers[i] = malloc(strlen(events_buffer[i].formatted_value) + 1);
			if(!str_identifiers[i]){
				fprintf(stderr, "Memory allocation failed for str_identifiers\n");
				return -1;
			}

			strcpy(str_identifiers[i], events_buffer[i].formatted_value);
