...
InfluxDBPoint **create_points_batch(struct event_t_formatted *events_buffer, int events_count)
{
	...
	for (int i = 0; i < events_count; i++) {
        ...
        TagInfluxDB tags = {
            .machine_id = strdup(events_buffer[i].machine_id),
            .interface = strdup(events_buffer[i].interface),
            .flowid = events_buffer[i].flowid
        };

        // Verifica se i tag sono stati allocati correttamente
        if (!tags.machine_id || !tags.interface) {
            fprintf(stderr, "Memory allocation failed for tags in event %d\n", i);
            // Libera i punti e le loro risorse allocate finora in caso di errore
            for (int j = 0; j < i; j++) {
                free_influxdb_point(points_batch[j]);
            }
            free(tags.machine_id);
            free(tags.interface);
            free(points_batch);
            return NULL;
        }
        ...
        // Controlla se la creazione del punto è riuscita
        if (!points_batch[i]) {
            fprintf(stderr, "Failed to create point for event %d\n", i);
            // Libera i punti e le loro risorse allocate finora in caso di errore
            for (int j = 0; j < i; j++) {
