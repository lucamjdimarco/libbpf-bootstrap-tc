...
void print_flow_id_info_ipv4_details(__u64 key, struct key_5tuple_ipv4 *value)
{
	printf("Flow: %llu\n", key);
	printf("---------------\n");
...
{
	__u64 *key, *prev_key;
	struct key_5tuple_ipv4 *value;
	int err;

	key = malloc(sizeof(__u64));
	prev_key = NULL;
	value = malloc(sizeof(struct key_5tuple_ipv4));
...
#ifdef CLASSIFY_IPV4
	struct key_5tuple_ipv4 *key, *prev_key;
	key = malloc(sizeof(struct key_5tuple_ipv4));
#endif
...
static int handle_event_rb2(void *ctx, void *data, size_t data_sz)
{
    ...
    struct key_5tuple_ipv4 value_ipv4;
    ...
    struct key_5tuple_ipv6 value_ipv6;
    value_size = sizeof(value_ipv6);
    flow_map_fd = bpf_map__fd(skel->maps.flow_info_ipv6);
    memset(&value_ipv6, 0, value_size);
    value_struct = &value_ipv6;

	#elif defined(CLASSIFY_ONLY_ADDRESS_IPV4)
    struct key_only_addr_ipv4 value_addr_ipv4;
    value_size = sizeof(value_addr_ipv4);
    flow_map_fd = bpf_map__fd(skel->maps.flow_info_only_addr_ipv4);
    memset(&value_addr_ipv4, 0, value_size);
    value_struct = &value_addr_ipv4;

	#elif defined(CLASSIFY_ONLY_ADDRESS_IPV6)
    struct key_only_addr_ipv6 value_addr_ipv6;
    key_size = sizeof(value_addr_ipv6);
    flow_id_map_fd = bpf_map__fd(skel->maps.flow_id_info_ipv6);
    memset(&value_addr_ipv6, 0, key_size);
    key_struct = &value_addr_ipv6;

	#elif defined(CLASSIFY_ONLY_DEST_ADDRESS_IPV4)
    struct key_only_dest_ipv4 value_dest_ipv4;
    key_size = sizeof(value_dest_ipv4);
    flow_id_map_fd = bpf_map__fd(skel->maps.flow_id_info_ipv4);
    memset(&value_dest_ipv4, 0, key_size);
    key_struct = &value_dest_ipv4;

	#elif defined(CLASSIFY_ONLY_DEST_ADDRESS_IPV6)
    struct key_only_dest_ipv6 value_dest_ipv6;
    key_size = sizeof(value_dest_ipv6);
    flow_id_map_fd = bpf_map__fd(skel->maps.flow_id_info_ipv6);
    memset(&value_dest_ipv6, 0, key_size);
    key_struct = &value_dest_ipv6;
